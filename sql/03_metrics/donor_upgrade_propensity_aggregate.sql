-- donor_upgrade_propensity_aggregate.sql
-- AGGREGATE VIEW
...
WHERE fy25.fy2025_total > fy24.fy2024_total  -- Only donors whose giving GREW
  AND (fy25.fy2025_total - fy24.fy2024_total) >= 500  -- Min $500 increase [filters can be adjusted]
  AND ((fy25.fy2025_total - fy24.fy2024_total) * 100.0 / fy24.fy2024_total) >= 20  -- Min 20% increase [filters can be adjusted]

GROUP BY fy24.current_rating, fy24.donor_type

HAVING COUNT(DISTINCT fy24.Donor_ID) >= 10 -- Filter out small samples. Only show segments with at least 10 donors who upgraded [governance]

ORDER BY fy24.current_rating, fy24.donor_type
